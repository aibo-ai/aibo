...
describe('CacheService', () => {
  ...
  beforeEach(() => {
    // Create mock methods as Jest functions
    const mockCreateIfNotExistsDb = jest.fn().mockResolvedValue({} as any);
    const mockCreateIfNotExistsContainer = jest.fn().mockResolvedValue({} as any);
    const mockContainer = jest.fn();
    const mockDatabase = jest.fn();
    
    mockCosmosClient = {
      databases: {
        createIfNotExists: mockCreateIfNotExistsDb
      },
      database: mockDatabase.mockReturnValue({
        containers: {
          createIfNotExists: mockCreateIfNotExistsContainer
        },
    ...
    cacheService = new CacheService(mockCosmosClient);
  });

  describe('initialize', () => {
    it('should initialize database and container', async () => {
      ...
      expect(mockCosmosClient.database('test-database').containers.createIfNotExists).toHaveBeenCalledWith({
        id: 'test-container',
        partitionKey: '/key',
        defaultTtl: 3600
      });
    ...
    it('should handle initialization errors', async () => {
      ...
      (mockCosmosClient.databases.createIfNotExists as jest.Mock).mockRejectedValue(error);
